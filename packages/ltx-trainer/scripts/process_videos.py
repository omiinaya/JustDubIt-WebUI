...
class MaskDataset(MediaDataset):
    ...
    This dataset extends MediaDataset to handle mask-specific processing:
    - Converts videos to grayscale masks
    - Applies mask-specific resizing to VAE latent dimensions
    - Preserves binary mask values using nearest neighbor interpolation
    - No normalization transforms (keeps values in [0,1] range)
    """
    ...
    def __init__(
        ...
        Args:
            dataset_file: Path to CSV/JSON/JSONL metadata file
            mask_column: Column name for mask video paths in the metadata file
            video_column: Column name for regular video paths (for reference)
            resolution_buckets: List of (frames, height, width) tuples
        """
        # Initialize parent class with mask_column as main_media_column
        # and no reshape mode (we'll handle resizing differently for masks)
        super().__init__(
            dataset_file=dataset_file,
            main_media_column=mask_column,
        ...
        # Remove the normalization transforms since masks don't need them
        # Also simplify the transform - just clamp and convert to grayscale
        self.transforms = transforms.Compose(
            [
                transforms.Lambda(lambda x: x.clamp_(0, 1)),
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        """Get a single mask with metadata."""
        if isinstance(index, list):
            # Special case for BucketSampler - return cached data
            return index

        # Try to load the requested sample, with fallback to other samples if it fails
        max_retries = 5
        ...
        for attempt in range(max_retries):
            ...
            mask_path: Path = self.main_media_paths[current_index]
            video_path: Path = self.video_paths[current_index]

            # Compute relative paths
            data_root = self.dataset_file.parent
            mask_relative_path = str(mask_path.relative_to(data_root))
            video_relative_path = str(video_path.relative_to(data_root))

            try:
                # Process the mask video (override parent's video processing)
                mask_tensor = self._preprocess_mask(mask_path)

                if attempt > 0:
                    logger.info(
                        f"Successfully loaded alternative sample {current_index} after {attempt} failed attempts"
                    )
                ...
                return {
                    ...
                    "mask_metadata": {
                        ...
                        "height": mask_tensor.shape[1],
                        "width": mask_tensor.shape[2],
                    },
                }
            except Exception as e:
                logger.warning(
    ...
    def _preprocess_mask(self, path: Path) -> torch.Tensor:
        ...
        # Resize to VAE latent dimensions
        mask_tensor = self._resize_masks(mask_tensor, reduce_type="max")
    ...
    def _resize_masks(
        ...
        """Resize masks to target resolution using bicubic interpolation.

        Args:
            masks: Binary masks tensor of shape [T, H, W]
            reduce_type: How to reduce temporal dimension
                - "max": Take maximum value across frames
                - "first": Take first frame
                - "last": Take last frame
                - "mean": Take mean across frames
                - "min": Take minimum value across frames

        Returns:
            Resized masks tensor of shape [T', H', W']
        """
        assert masks.dim() == 3, "Masks must have 3 dimensions [T, H, W]"

        num_frames, height, width = masks.shape
        target_height = height // VAE_SPATIAL_FACTOR
        target_width = width // VAE_SPATIAL_FACTOR
        target_frames = 1 + (num_frames - 1) // VAE_TEMPORAL_FACTOR
        # First resize spatial dimensions
        resized = (
            resize(
                masks.unsqueeze(1),  # Add channel dim: [T, 1, H, W]
                [target_height, target_width],
                interpolation=InterpolationMode.NEAREST,
            )
            .squeeze(1)
            .clip(0, 1)
        )  # Remove channel dim and clip: [T, H, W]

        # Keep first frame and process remaining frames
        first_frame = resized[:1]  # [1, H, W]
        remaining_frames = resized[1:]  # [T-1, H, W]

        # Calculate how many groups of 8 frames we need to reduce to (target_frames - 1)
        target_remaining = target_frames - 1

        if target_remaining <= 0 or remaining_frames.shape[0] == 0:
        ...
        if reduce_type == "max":
            remaining_frames = grouped_frames.max(dim=1).values
        elif reduce_type == "mean":
            remaining_frames = grouped_frames.mean(dim=1)
        elif reduce_type == "min":
            remaining_frames = grouped_frames.min(dim=1).values
        else:
...
def decode_masks_folder(masks_dir: Path, output_dir: Path) -> None:
    ...
    masks_files = list(masks_dir.rglob("*.pt"))
    ...
    with Progress(
        ...
        task = progress.add_task("Decoding latents masks", total=len(masks_files))
        ...
        for masks_file in masks_files:
            try:
                ...
                is_image = mask_video.shape[0] == 1
                if is_image:
                    # Save as PNG for single frame
                    output_path = output_dir / f"{masks_file.stem}.png"
                    torchvision.utils.save_image(
                        mask_video[0] / 255.0,  # [H,W,C] -> [C,H,W] and normalize
                        str(output_path),
                    )
                else:
                    # Save as MP4 for video
                    output_path = output_dir / f"{masks_file.stem}.mp4"
                    fps = data.get("fps", 24)  # Use stored FPS or default to 24
                    # Convert grayscale mask to RGB: [T, H, W] -> [T, H, W, 3]
                    mask_video_rgb = mask_video.unsqueeze(-1).repeat(1, 1, 1, 3)
                ...
                logger.error(f"Error processing {masks_file}: {e}")
                continue

        progress.advance(task)

    logger.info(f"Mask decoding complete! Videos saved to {output_dir}")
...
def decode_mask(
    ...
    Simple inverse: upsample spatially and repeat each frame (except first) 8 times.

    Args:
        masks: Latent masks tensor of shape [T', H', W']
        target_height: Target height
        target_width: Target width

    Returns:
        Decoded masks tensor upsampled to [T_expanded, target_height, target_width]
    """
    assert masks.dim() == 3, "Masks must have 3 dimensions [T', H', W']"
    ...
    upsampled = (
        resize(
            masks.unsqueeze(1),  # Add channel dim: [T', 1, H', W']
            [target_height, target_width],
            interpolation=InterpolationMode.NEAREST,  # Preserve mask values
        )
        .squeeze(1)
        .clip(0, 1)
    ...
    first_frame = upsampled[:1]  # [1, H, W]
    remaining_frames = upsampled[1:]  # [T'-1, H, W]

    # Repeat each remaining frame 8 times
    if remaining_frames.shape[0] > 0:
        expanded_frames = []
        for i in range(remaining_frames.shape[0]):
            frame = remaining_frames[i : i + 1]  # [1, H, W]
            repeated = frame.repeat(8, 1, 1)  # [8, H, W]
            expanded_frames.append(repeated)

        expanded = torch.cat(expanded_frames, dim=0)  # [8*(T'-1), H, W]

        # Combine first frame with expanded frames
        return torch.cat([first_frame, expanded], dim=0)  # [1 + 8*(T'-1), H, W]
    else:
        # Only first frame
        return first_frame
...
def compute_mask_latents(
    ...
    """
    Process mask videos and save them in latent-compatible format.

    Args:
        dataset_file: Path to metadata file containing mask paths
        mask_column: Column name for mask video paths
        video_column: Column name for regular video paths (for reference)
        resolution_buckets: List of (frames, height, width) tuples
    ...
    logger.info(f"Loading dataset from {dataset_file} with mask column {mask_column}")
    # Create dataset
    dataset = MaskDataset(
        dataset_file=dataset_file,
    ...
    logger.info(f"Loaded {len(dataset)} valid mask files")
    ...
    with Progress(
        ...
        task = progress.add_task("Processing masks", total=len(dataloader))

        for batch in dataloader:
            try:
                # Check if we should skip existing files
                if batch_size == 1:
                    video_relative_path = batch["video_relative_path"][0]
                    output_rel_path = Path(video_relative_path).with_suffix(".pt")
                    output_file = output_path / output_rel_path
                    if output_file.exists() and not override:
                        progress.advance(task)
                        continue

                # Save masks for each item in batch
                for i in range(len(batch["mask_relative_path"])):
                    video_relative_path = batch["video_relative_path"][i]
                    output_rel_path = Path(video_relative_path).with_suffix(".pt")
                    output_file = output_path / output_rel_path
                    ...
                    torch.save(mask_data, output_file)
...
def encode_audio(
    ...
    if mel_spectrogram.shape[1] == 1:
        mel_spectrogram = torch.cat([mel_spectrogram, mel_spectrogram], dim=1)
...
def main(  # noqa: PLR0913
    ...
    resolution_buckets: str = typer.Option(
        ...,
        help='Resolution buckets in format "WxHxF" or "WxH". Use "WxHxF" for fixed frames (e.g. "768x768x25"), '
        'or "WxH" to auto-select best 1+8*N frames per video (e.g. "1280x720")',
    ),
