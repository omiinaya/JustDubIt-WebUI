...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Reference audio/video for audio-video-to-audio-video generation (AV-to-AV mode)",
    )
    parser.add_argument(
        "--include-reference-in-output",
        action="store_true",
        help="Include reference video side-by-side with generated output (only for V2V/AV2AV)",
    )
    parser.add_argument(
        "--include-diff-with-reference-in-output",
        action="store_true",
        help="Include difference in pixel space between reference and generated output (only for V2V/AV2AV)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Input JSON file containing reference videos and prompts",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    if args.input_json is not None:
        ...
        with open(args.input_json, "r") as f:
            data = json.load(f)
        data_root = Path(args.input_json).parent
        reference_videos = [data_root / Path(item["reference_path"]) for item in data]
        prompts = [data_root / Path(item["caption_path"]) for item in data]
    ...
    for sample_idx, (reference_video, prompt) in enumerate(
        ...
        args.num_frames = num_frames[sample_idx]
        args.output = Path(args.output_dir) / f"sample_{sample_idx}.mp4"
        args.audio_output = None
        ...
        if args.reference_video is not None and args.reference_audio:
            mode = "Audio-Video-to-Audio-Video (AV2AV)"
        elif args.reference_video is not None and args.condition_image is not None:
        ...
        gen_config = GenerationConfig(
            ...
            include_diff_with_reference_in_output=args.include_diff_with_reference_in_output,
