...
def preprocess_dataset(  # noqa: PLR0912, PLR0913, PLR0915
    ...
    if reference_column:
        ...
        reference_audio_latents_dir = None
        if with_audio:
            logger.info("Processing reference audio for IC-LoRA training...")
    ...
    if decode:
        ...
        logger.info("Decoding target latents...")
        ...
        if mask_column:
            ...
            if masks_dir.exists():
                logger.info("Decoding mask latents...")
                decode_masks_folder(masks_dir, output_base / "decoded_masks")
...
def main(  # noqa: PLR0913
    ...
    mask_column: str | None = typer.Option(
        default=None,
        help="Column name containing mask paths in the dataset JSON/JSONL/CSV file",
    ),
) -> None:
