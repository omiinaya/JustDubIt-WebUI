...
TrainingStrategyConfig = Annotated[
    ...
    | Annotated[JustDubItConfig, Tag("justdubit")],
...
class OptimizationConfig(ConfigBaseModel):
    ...
    learning_rate_groups: dict[str, float] | None = Field(
        default=None,
        description="Optional mapping of parameter name patterns (regex) to learning rates. "
        "If a parameter name matches multiple patterns, the first one is used.",
    )
