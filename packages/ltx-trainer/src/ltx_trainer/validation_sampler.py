...
class ValidationSampler:
    ...
    def _generate_with_reference_av(self, config: GenerationConfig, device: torch.device) -> tuple[Tensor, Tensor | None]:
        ...
        combined_audio_clean_state = LatentState(
            ...
            denoise_mask=torch.cat([ref_audio_denoise_mask, audio_clean_state.denoise_mask], dim=1),
            positions=torch.cat([ref_audio_positions, audio_clean_state.positions], dim=2),
            clean_latent=torch.cat([ref_audio_latent, audio_clean_state.clean_latent], dim=1),
        )
        combined_audio_state = noiser(latent_state=combined_audio_clean_state, noise_scale=1.0)
    ...
    def _encode_audio(
        ...
        positions = self._audio_patchifier.get_patch_grid_bounds(output_shape=latent_shape, device=device)
        positions = positions.to(torch.bfloat16)
    ...
    @staticmethod
    def _compute_video_diff(ref_video: torch.Tensor, gen_video: torch.Tensor) -> torch.Tensor:
        """
