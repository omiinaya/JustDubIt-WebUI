...
class TiledDecodingConfig:
    ...
    tile_size_pixels: int = (
        192  # Spatial tile size in pixels (must be ≥64 and divisible by 32)
    )
    tile_overlap_pixels: int = (
        64  # Spatial tile overlap in pixels (must be divisible by 32)
    )
    tile_size_frames: int = (
        48  # Temporal tile size in frames (must be ≥16 and divisible by 8)
    )
    tile_overlap_frames: int = (
        24  # Temporal tile overlap in frames (must be divisible by 8)
    )
...
class GenerationConfig:
    ...
    condition_image: Tensor | None = (
        None  # Optional first frame image for image-to-video
    )
    ...
    include_reference_in_output: bool = (
        False  # For IC-LoRA: concatenate original reference with generated output
    )
    cached_embeddings: CachedPromptEmbeddings | None = (
        None  # Pre-computed text embeddings (avoids loading Gemma)
    )
    stg_scale: float = 0.0  # STG strength (0.0 = disabled, recommended: 1.0)
    stg_blocks: list[int] | None = (
        None  # Transformer blocks to perturb (None = all, recommended: [29])
    )
    stg_mode: Literal["stg_av", "stg_v"] = (
        "stg_av"  # STG mode: "stg_av" (audio+video) or "stg_v" (video only)
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.tiled_decoding is None:
            ...
            object.__setattr__(
                self, "tiled_decoding", TiledDecodingConfig(enabled=False)
            )
...
class ValidationSampler:
    ...
    def __init__(
        ...
        Args:
            ...
            audio_encoder: Optional audio VAE encoder (for audio conditioning)
    ...
    def generate(
        ...
        if config.reference_audio is not None and config.reference_video is not None:
            return self._generate_with_reference_av(config, device)
    ...
    def _generate_standard(
        ...
        v_ctx_pos, a_ctx_pos, v_ctx_neg, a_ctx_neg = self._get_prompt_embeddings(
            config, device
        )
        ...
        audio_tools = (
            self._create_audio_latent_tools(config) if config.generate_audio else None
        )

        # Create initial states
        video_clean_state = video_tools.create_initial_state(
    ...
    def _generate_with_reference(
        ...
        ref_latent, ref_positions = self._encode_video(
            ref_video_preprocessed, config.frame_rate, device
        )
        ref_seq_len = ref_latent.shape[1]

        # Create target video state
        video_tools = self._create_video_latent_tools(config)
        target_clean_state = video_tools.create_initial_state(
            device=device, dtype=torch.bfloat16
        )
        ...
        if config.include_reference_in_output:
            ...
            video_output = self._concatenate_videos_side_by_side(
                ref_video_pixels, video_output
            )
    ...
    def _generate_with_reference_av(
        ...
        - Reference video latents are concatenated with target latents
        - Reference audio latents are concatenated with target audio latents
        - Reference latents have timestep=0 (clean, not denoised)
        - Target latents are denoised normally
        - If condition_image is also provided, the first frame of the target is conditioned
        - Does not support concatenating original reference video side-by-side with the generated video
        """
        # Encode prompts
        v_ctx_pos, a_ctx_pos, v_ctx_neg, a_ctx_neg = self._get_prompt_embeddings(
            config, device
        ...
        # Create combined video state (reference + target)
        combined_clean_state = LatentState(
            latent=torch.cat([ref_latent, target_clean_state.latent], dim=1),
            denoise_mask=torch.cat(
                [ref_denoise_mask, target_clean_state.denoise_mask], dim=1
        ...
        ref_audio_latent, ref_audio_positions, unpatchified_ref_audio_shape = (
            self._encode_audio(ref_audio_preprocessed, device=device)
        )
        ref_audio_seq_len = ref_audio_latent.shape[1]
        ref_audio_denoise_mask = torch.zeros(
            1, ref_audio_seq_len, 1, device=device, dtype=torch.float32
        )

        # Create audio state if needed
        audio_tools = (
            self._create_audio_latent_tools(
                config, reference_shape=unpatchified_ref_audio_shape
            )
        ...
        combined_audio_clean_state = LatentState(
            latent=torch.cat([ref_audio_latent, audio_clean_state.latent], dim=1),
            denoise_mask=torch.cat(
                [ref_audio_denoise_mask, audio_clean_state.denoise_mask], dim=1
            ),
            positions=torch.cat(
                [ref_audio_positions, audio_clean_state.positions], dim=2
            ),
            clean_latent=torch.cat(
                [ref_audio_latent, audio_clean_state.clean_latent], dim=1
            ),
        )
        combined_audio_state = noiser(
            latent_state=combined_audio_clean_state, noise_scale=1.0
        )

        # Run denoising loop
        combined_state, combined_audio_state = self._run_denoising(
            config=config,
            video_state=combined_state,
            audio_state=combined_audio_state,
            video_clean_state=combined_clean_state,
            audio_clean_state=combined_audio_clean_state,
            v_ctx_pos=v_ctx_pos,
            a_ctx_pos=a_ctx_pos,
            v_ctx_neg=v_ctx_neg,
            a_ctx_neg=a_ctx_neg,
        ...
        # Optionally concatenate original reference video side-by-side
        assert not (
            config.include_reference_in_output
            and config.include_diff_with_reference_in_output
        ...
        if config.include_diff_with_reference_in_output:
            # Calculate difference in pixel space between reference and generated output
            ref_video_pixels = ref_video_preprocessed[0].cpu()
            # Normalize from [-1, 1] to [0, 1]
            ref_video_pixels = ((ref_video_pixels + 1.0) / 2.0).clamp(0.0, 1.0)
            diff = self._compute_video_diff(ref_video_pixels, video_output)
            video_output = self._concatenate_videos_side_by_side(diff, video_output)

        # Extract target audio portion and decode
        target_audio_latent = combined_audio_state.latent[:, ref_audio_seq_len:]
        target_audio_state = LatentState(
            latent=target_audio_latent,
            denoise_mask=audio_clean_state.denoise_mask,
            positions=audio_clean_state.positions,
            clean_latent=audio_clean_state.clean_latent,
        )
        target_audio_state = audio_tools.unpatchify(target_audio_state)
        audio_output = self._decode_audio(target_audio_state, device)
    ...
    def _create_audio_latent_tools(
        ...
        if reference_shape:
            return AudioLatentTools(
                ...
                target_shape=AudioLatentShape.from_torch_shape(reference_shape),
            )

        return AudioLatentTools(
            patchifier=self._audio_patchifier,
            target_shape=AudioLatentShape.from_duration(
                batch=1, duration=config.num_frames / config.frame_rate
            ),
        )
    ...
    def _apply_image_conditioning(
        ...
        encoded_image = self._encode_conditioning_image(
            image, config.height, config.width, device
        )

        # Patchify the encoded image (single frame)
        patchified_image = self._video_patchifier.patchify(
            encoded_image
        )  # [1, 1, C] -> [1, num_patches, C]
        ...
        new_clean_latent[:, :num_image_tokens] = patchified_image.to(
            new_clean_latent.dtype
        )
    ...
    def _preprocess_reference_video(config: GenerationConfig) -> Tensor:
        ...
        if current_height != target_height or current_width != target_width:
            ...
            if aspect_ratio > target_aspect_ratio:
                resize_height, resize_width = target_height, int(
    ...
    def _encode_video(
        self, video: Tensor, fps: float, device: torch.device
    ) -> tuple[Tensor, Tensor]:
        ...
        with torch.autocast(
            device_type=str(device).split(":")[0], dtype=torch.bfloat16
        ):
        ...
        positions = get_pixel_coords(
            latent_coords, scale_factors=VIDEO_SCALE_FACTORS, causal_fix=True
        )
    ...
    def _preprocess_reference_audio(
        ...
        # trim reference audio to valid duration
    ...
    def _encode_audio(
        ...
        Returns:
            Tuple of (latents, positions),
            latents: [batch, channels, time, freq]
            positions: [batch, 1, time, 2]
        """
        waveform = audio_data["waveform"].to(device=device, dtype=torch.float32)
        ...
        mel_spectrogram = self._audio_processor.waveform_to_mel(
            waveform, waveform_sample_rate=sample_rate
        )
        mel_spectrogram = mel_spectrogram.to(dtype=torch.bfloat16)
        self._audio_processor.to("cpu")

        # AudioVAE.encode expects {"waveform": Tensor[batch, channels, samples], "sample_rate": int}
        self._audio_encoder.to(device)
        with torch.autocast(
            device_type=str(device).split(":")[0], dtype=torch.bfloat16
        ):
            audio_latents = self._audio_encoder(mel_spectrogram)
        self._audio_encoder.to("cpu")

        # latents shape: [batch, channels, time, freq] = [1, 8, T, 16]
        batch_size, channels, time_steps, freq_bins = audio_latents.shape
        patchified_audio_latents = self._audio_patchifier.patchify(audio_latents)
    ...
    def _preprare_cross_attention_mask(
        ...
        video_mask = video_state.denoise_mask
        audio_mask = audio_state.denoise_mask
    ...
    def _run_denoising(
        ...
        stg_perturbation_config = (
            self._build_stg_perturbation_config(config)
        ...
        if config.enable_cross_attention_masking:
            v2a_cross_attention_mask, a2v_cross_attention_mask = (
                self._preprare_cross_attention_mask(video_state, audio_state, device)
            )
        else:
        ...
        video = Modality(
            ...
            context=v_ctx_pos,
            context_mask=None,
            cross_attention_mask=a2v_cross_attention_mask,
        )
        ...
        if audio_state is not None:
            audio = Modality(
                ...
                context=a_ctx_pos,
                context_mask=None,
                cross_attention_mask=v2a_cross_attention_mask,
            )
        ...
        with torch.autocast(
            ...
            for step_idx, sigma in enumerate(sigmas[:-1]):
                ...
                pos_video, pos_audio = x0_model(
                    video=video, audio=audio, perturbations=None
                )
                ...
                if cfg_guider.enabled() and v_ctx_neg is not None:
                    ...
                    audio_neg = (
                        replace(audio, context=a_ctx_neg) if audio is not None else None
                    )
                    ...
                    if audio is not None and denoised_audio is not None:
                        denoised_audio = denoised_audio + cfg_guider.delta(
                            pos_audio, neg_audio
                        )
                ...
                if (
                    ...
                    and audio_clean_state is not None
                ):
                    denoised_audio = (
                        denoised_audio * audio_state.denoise_mask
                        + audio_clean_state.latent.float()
    ...
    def _build_stg_perturbation_config(
        ...
        perturbations: list[Perturbation] = [
            Perturbation(
                type=PerturbationType.SKIP_VIDEO_SELF_ATTN, blocks=config.stg_blocks
            )
        ]
        ...
        if config.stg_mode == "stg_av":
            perturbations.append(
                Perturbation(
                    type=PerturbationType.SKIP_AUDIO_SELF_ATTN, blocks=config.stg_blocks
                )
            )
    ...
    def _decode_video_latent(
        self, latent: Tensor, config: GenerationConfig, device: torch.device
    ) -> Tensor:
    ...
    def _validate_config(self, config: GenerationConfig) -> None:
        ...
        if config.height % 32 != 0 or config.width % 32 != 0:
            raise ValueError(
                f"height and width must be divisible by 32, got {config.height}x{config.width}"
            )
        if config.num_frames % 8 != 1:
            raise ValueError(
                f"num_frames must satisfy num_frames % 8 == 1, got {config.num_frames}"
            )
        if config.generate_audio and (
            self._audio_decoder is None or self._vocoder is None
        ):
        ...
        if config.cached_embeddings is None and self._text_encoder is None:
            raise ValueError(
                "Either text_encoder or config.cached_embeddings must be provided"
            )
    ...
    def _decode_video(
        ...
        video_state: LatentState,
        device: torch.device,
        tiled_config: TiledDecodingConfig | None = None,
    ) -> Tensor:
    ...
    @staticmethod
    def _concatenate_videos_side_by_side(
        left_video: Tensor, right_video: Tensor
    ) -> Tensor:
        ...
        if left_frames < right_frames:
            padding = left_video[:, -1:, :, :].expand(
                -1, right_frames - left_frames, -1, -1
            )
            left_video = torch.cat([left_video, padding], dim=1)
        elif right_frames < left_frames:
            padding = right_video[:, -1:, :, :].expand(
                -1, left_frames - right_frames, -1, -1
            )
    ...
    @staticmethod
    def _compute_video_diff(
        ref_video: torch.Tensor, gen_video: torch.Tensor
    ) -> torch.Tensor:
        """
        Focuses on structural contours (edges) while ignoring color and pixel noise.
        Input: [C, F, H, W] in [0, 1] range.
        """
        # [C, F, H, W] -> [F, H, W, C]
        ...
        for i in range(ref_np.shape[0]):
            ...
            ref_gray = cv2.cvtColor(
                (ref_np[i] * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY
            )
            gen_gray = cv2.cvtColor(
                (gen_np[i] * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY
            )
            ...
            diff_mask = np.where(struct_diff > threshold, struct_diff, 0).astype(
                np.float32
            )
        ...
        # Convert back to [C, F, H, W]
        diff_out = np.stack(diff_frames, axis=0)
        return torch.from_numpy(diff_out).permute(3, 0, 1, 2).to(gen_video.device)
