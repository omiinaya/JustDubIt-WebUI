...
class LtxvTrainer:
    ...
    def _load_models(self) -> None:
        ...
        need_audio_vae_encoder = load_audio and need_vae_encoder
        ...
        components = load_ltx_model(
            ...
            with_audio_processor=need_audio_vae_encoder,
            with_audio_vae_encoder=need_audio_vae_encoder,
        ...
        self._audio_processor = components.audio_processor
        self._audio_vae_decoder = components.audio_vae_decoder
        self._audio_vae_encoder = components.audio_vae_encoder
        ...
        if self._audio_vae_decoder is not None:
            self._audio_vae_decoder.requires_grad_(False)
        if self._audio_vae_encoder is not None:
    ...
    def _init_optimizer(self) -> None:
        ...
        # Support for different learning rates for different parameters
        if opt_cfg.learning_rate_groups:
            # Map patterns to their learning rates
            group_configs = opt_cfg.learning_rate_groups

            # Initialize groups: index 0 is the default group
            param_groups = [{"params": [], "lr": lr, "names": []}]
            pattern_to_group_idx = {}

            for i, (pattern, group_lr) in enumerate(group_configs.items()):
                param_groups.append({"params": [], "lr": group_lr, "names": []})
                pattern_to_group_idx[pattern] = i + 1

            # Assign parameters to groups based on name matching
            # Note: self._transformer might be wrapped in PEFT/DDP/FSDP
            for name, param in self._transformer.named_parameters():
                if not param.requires_grad:
                    continue

                assigned = False
                for pattern, group_idx in pattern_to_group_idx.items():
                    if re.search(pattern, name):
                        param_groups[group_idx]["params"].append(param)
                        param_groups[group_idx]["names"].append(name)
                        assigned = True
                        break
            ...
            # Remove any empty groups and use the remaining as optimizer parameters
    ...
    def _sample_videos(self, progress: TrainingProgress) -> list[Path] | None:
        ...
        for prompt_idx, prompt in enumerate(self._config.validation.prompts):
            ...
            gen_config = GenerationConfig(
                ...
                enable_cross_attention_masking=enable_cross_attention_masking,
