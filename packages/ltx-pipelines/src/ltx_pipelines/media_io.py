...
def resize_and_center_crop(
    ...
    """
    if latent.ndim == 3:
        latent = rearrange(latent, "h w c -> 1 c h w")
    elif latent.ndim == 4:
    ...
    latent = latent[:, :, crop_top : crop_top + height, crop_left : crop_left + width]

    latent = rearrange(latent, "f c h w -> 1 c f h w")
    return latent
...
def encode_video(
    ...
    for video_chunk in all_tiles(first_chunk, video):
