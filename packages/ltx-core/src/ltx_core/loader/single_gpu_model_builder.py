...
class SingleGPUModelBuilder(
    Generic[ModelType], ModelBuilderProtocol[ModelType], LoRAAdaptableProtocol
):
    """
    ...
    model_loader: StateDictLoader = field(
        default_factory=SafetensorsModelStateDictLoader
    )
    ...
    def lora(
        ...
        return replace(
            self,
            loras=(*self.loras, LoraPathStrengthAndSDOps(lora_path, strength, sd_ops)),
        )
    ...
    def _return_model(self, meta_model: ModelType, device: torch.device) -> ModelType:
        ...
        if uninitialized_params or uninitialized_buffers:
            logger.warning(
                f"Uninitialized parameters or buffers: {uninitialized_params + uninitialized_buffers}"
            )
            return meta_model
    ...
    def build(
        ...
        model_paths = (
            self.model_path if isinstance(self.model_path, tuple) else [self.model_path]
        )
        model_state_dict = self.load_sd(
            model_paths, sd_ops=self.model_sd_ops, registry=self.registry, device=device
        )

        lora_strengths = [lora.strength for lora in self.loras]
        if not lora_strengths or (
            min(lora_strengths) == 0 and max(lora_strengths) == 0
        ):
        ...
        lora_state_dicts = [
            self.load_sd(
                [lora.path], sd_ops=lora.sd_ops, registry=self.registry, device=device
            )
        ...
        final_sd = apply_loras(
            ...
            dtype=dtype,
            destination_sd=(
                model_state_dict if isinstance(self.registry, DummyRegistry) else None
            ),
        )
