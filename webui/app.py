...
def dub_video():
    ...
    import threading
    thread = threading.Thread(target=process_video, args=(job_id, filepath, output_path, prompt, height, width, steps, cfg, fps, seed))
    thread.daemon = True
    thread.start()
    
    return jsonify({'job_id': job_id, 'status': 'queued'})

def process_video(job_id, input_path, output_path, prompt, height, width, steps, cfg, fps, seed):
...
def delete_job(job_id):
    ...
    # Delete files
    input_path = os.path.join(UPLOAD_FOLDER, job['input_file'])
    output_path = os.path.join(OUTPUT_FOLDER, job['output_file'])
...
if __name__ == '__main__':
    print("Starting JustDubIt WebUI...")
    print("Open http://192.168.1.16:5000 in your browser")
